...
There are four commands: color, image, text and config.
You can use \"apex7tkl [command]\" for more details."
       "apex7tkl"))
...
(cl-argparse:add-subparser
 *arg-parser*
 (cl-argparse:create-sub-parser (image "Display an image in the OLED screen. Ideal images are 128x40, monochrome.
If a color image is used, it will be displayed using the average color as threshold for b/w.
Larger images are clipped, smaller ones are displayed in the top left corner.")
...
Keep in mind the screen fits about 21 characters, the rest won't be visible.")
...
(cl-argparse:add-subparser
 *arg-parser*
 (cl-argparse:create-sub-parser (config "Change to the N config for the keyboard.
Valid configs are in the range 1-5.")
   (cl-argparse:add-default config
                             :var "command"
                             :default #'cli-config)
   (cl-argparse:add-positional config
                               :name "config-number"
                               :help "The config number, 1 through 5.")))
